...
请使用 Markdown 格式输出，包含必要的小节、要点和示例。"""
    ...
    def __init__(
        ...
        Args:
            ...
            use_llm: Enable LLM generation (requires ANTHROPIC_API_KEY)
        ...
        self._api_available = self._check_api_key() if use_llm else False
        self.use_llm = use_llm and self._api_available
        
        if use_llm and not self._api_available:
            logger.warning(
                "LLM mode requested but ANTHROPIC_API_KEY not set. "
